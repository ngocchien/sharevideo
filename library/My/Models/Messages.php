...
class Messages extends ModelAbstract {
    ...
    private function getParentTable() {
        ...
        return new \My\Storage\storageMessages($dbAdapter);
    }

    public function __construct() {
        $this->setTmpKeyCache('tmpMessages');
        parent::__construct();
    }

    public function getList($arrCondition = array()) {
        $keyCaching = 'getListMessages' . $this->cache->read($this->tmpKeyCache);
    ...
    public function getListLimit($arrCondition = [], $intPage = 1, $intLimit = 15, $strOrder = 'mess_id ASC') {
        $keyCaching = 'getListLimitMessages:' . $intPage . ':' . $intLimit . ':' . str_replace(' ', '_', $strOrder) . ':' . $this->cache->read($this->tmpKeyCache);
        if (count($arrCondition) > 0) {
            foreach ($arrCondition as $k => $val) {
                $keyCaching .= $k . ':' . $val . ':';
            }
